...
def main():
    ...
    times = deque(maxlen=20)  # Historical times for a moving average
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in as_completed(future_to_file):
            ...
            times.append(elapsed_time / processed_files)  # Time per file (moving average)
            average_time_per_file = sum(times) / len(times)
            estimated_time_remaining = average_time_per_file * (num_lines - processed_files)
